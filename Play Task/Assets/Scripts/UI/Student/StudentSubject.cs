...
public class StudentSubject : StudentDashboardSubjects
{
    ...
    private void DisplayStudentList(ScrollView list, Dictionary<string, string> topicData, int number)
    {
        ...
        newItem.RegisterCallback<MouseUpEvent>(evt => {
            foreach (Dictionary<string, string> topic in topicList)
            {
                ...
                {
                    GetComponent<StudentSubtopic>().selectedTopicId = topic["Id"];
                    GetComponent<StudentSubtopic>().selectedTopicTitle = topic["Title"];

                    GetComponent<StudentSubtopic>().GetSubtopics(topic["Id"]);
                }
            }
        });
    }
